...
impl StarvationDiagnostic {
    ...
    /// Create a diagnostic repair bead when unrecoverable conditions are detected
    fn create_repair_bead(&self, report: &StarvationDiagnosticReport) -> Result<String> {
        ...
        let title = format!(
            "Auto-escalation: Starvation repair - {}",
            report.summary.primary_cause.replace('_', " ")
        );

        let mut description = format!(
            "## Auto-Escalated Starvation Repair\n\n\
            **Generated by:** icg-starvation-diagnostic\n\
            **Timestamp:** {}\n\
            **Workspace:** {}\n\
            **Primary Cause:** {}\n\n",
        ...
        description.push_str("## Diagnostic Findings\n\n");

        // Database integrity
        description.push_str("### Database Integrity\n");
        if report.integrity_check.corruption_detected {
            description.push_str("❌ **CORRUPTION DETECTED**\n");
            for issue in &report.integrity_check.issues {
                description.push_str(&format!("- {}\n", issue));
            }
        } else {
            description.push_str("✅ Database integrity OK\n");
        }
        description.push_str("\n");

        // Checkpoint status
        description.push_str("### Checkpoint Status\n");
        description.push_str(&format!("- Status: {}\n", report.checkpoint_status.sync_status));
        description.push_str(&format!("- Exists: {}\n", report.checkpoint_status.checkpoint_exists));
        if let Some(minutes) = report.checkpoint_status.stale_minutes {
            description.push_str(&format!("- Stale by: {} minutes\n", minutes));
        }
        ...
        if !report.stale_assignees.is_empty() {
            description.push_str("### Stale Assignees Detected\n");
        ...
        // Add attempted repairs
        if !report.repairs_performed.is_empty() {
            description.push_str("## Attempted Repairs\n\n");
            description.push_str(&format!("Auto-repair performed: {} actions\n", report.repairs_performed.len()));
        ...
        // Add next steps
        description.push_str("## Next Steps\n\n");
        for (i, action) in report.recommended_actions.iter().enumerate() {
            description.push_str(&format!("{}. {}\n", i + 1, action));
        }

        description.push_str("\n**This bead was auto-generated by the starvation diagnostic system.**\n");
        description.push_str("**An agent should claim this bead and execute the recommended repair actions.**\n");
        ...
        let output = Command::new("bead")
            .args([
                "create",
                "--title", &title,
                "--priority", "2", // P2 for diagnostic repairs
                "--issue-type", "task",
                "--label", "auto-escalated",
                "--label", "starvation-repair",
        ...
        // Now add the description via bead update
        ...
        if !update_output.status.success() {
            ...
            eprintln!("Warning: Failed to update bead description: {}", stderr);
        }
