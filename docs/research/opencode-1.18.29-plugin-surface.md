...
| A **named export rides next to the default** (`export const x = …` plus `export default fn`) — the export check reports the module as non-function even though `default` is a function | same logError + skip. **Added 2026-09-20 during irrevers-1eac03df**, live against the pinned binary: five shapes probed in one bootstrap run — inline default arrow, named-const default, `import type` + annotation, and a `node:child_process` import all load fine; adding one `export const` beside the default fails with `error="Plugin export is not a function"`. The ICG plugin therefore ships **exactly one export**, its test surface riding on the factory as properties. The same row re-observed 2026-09-20 by the deployment verification (bead `irrevers-bba8bcc9`): a factory-throw canary at the deployed global path logged `failed to load plugin path=file:///home/coding/.config/opencode/plugin/icg.ts error="icg mount probe"` | live: `/tmp/probe-run.log` shapes reproduced twice; deployment probe in the bead notes |
