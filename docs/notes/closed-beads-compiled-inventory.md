...
| irrevers-e2bb8fbf | Add --channel to icg trust to match icg update (canary rollout) | 2026-08-30 12:19 | `c7e9df5` is the exact close-note claim: `--channel` help hint + `maintenance_scenario_trust_channel_roundtrip` (tests/maintenance_tasks_tests.rs:450 at HEAD); later `890429f`, earlier related `0fb164d` | GIT | solid |
...
| irrevers-c87a3c50 | Base self-updater and trust pointer (icg update) | 2026-08-14 14:37 | Closed 1 s after creation, supersede-only close reason, no commit/test; repo verifiably held none of the described code at close (tip `200bb7e`; trust_pointer.rs first appears `97b8eba` ~12.4 h later); scope lives under successors 5fdc2e13/f59f9313 | NONE | **weak-unverifiable (T1)** — void rehydration bookkeeping |
...
| irrevers-ff4f17da | Poison-pill auto-rollback: revert the trust pointer on a deny-rate spike | 2026-08-21 01:05 | `d653ade` 20 s before close: src/rollback.rs +371 (`check_and_rollback`) + runbook updates; four conservative-trigger tests at HEAD | GIT | solid |
...
| irrevers-9007792b | Operational monitoring and alerting infrastructure | 2026-08-21 03:28 | `a750033` 10 s before close, +1268 (Grafana dashboard, Prometheus alerts/scrape, promtail, monitor loop); tests :470/:545, src/denial_log.rs:1158 | GIT | solid |
...
| irrevers-f61efd80 | Layer 1: coverage-diff CI gate | 2026-08-15 03:40 | `97b8eba` created coverage.rs + coverage-diff CLI + tests 36 min before close, but the described report-format/justification scope landed 10 h 01 m **after** close (`48d5a60`); CI wiring `d11a6472` | GIT | **partial (T3)** — close predates full described scope |
...
- **irrevers-f61efd80** — the described report-format/justification scope
  landed ~10 h after close (`48d5a60`); the close-time anchor `97b8eba`
...
## Certification addendum — irrevers-2c3b4637 acceptance criteria (child 5, irrevers-fc249ab4)

Certified 2026-09-12 at repo HEAD `3f0861f`, completing the auto-split chain
(irrevers-3828115c → 98e9188e → 25063548 → 92165552 → fc249ab4). Every one of
the four parent acceptance criteria is satisfied; the one-to-one mapping:

| # | Parent criterion | Satisfied by |
|---|---|---|
| 1 | List of relevant closed beads with ID, title, close date | **This file, §A/§B/§C tables** — 42 rows, each carrying ID, Title, Close (UTC) (forensic-log close-instant per the enumeration, not the mass-drifted `updated_at`); rows verbatim from `docs/notes/closed-beads-release-verification-inventory.md`. Re-certified live: all 42 IDs still `closed` (`bead list`, 2026-09-12, HEAD `3f0861f`). |
| 2 | Per bead, a summary of the closing evidence (commits/tests/PRs where possible) | **This file, §A/§B/§C "Evidence summary" column** (one-line condensation per row with commit/test anchors); full entries in `docs/notes/evidence/final-inventory.md` §A/§B/§C (first pass, irrevers-f7a52307) and `docs/notes/evidence/second-pass-closing-evidence.md` §1 (irrevers-49dbb095 extraction + independent re-verification of the inherited 41 at `c4385ac`). No PRs exist anywhere in the inventory — commits are the unit of record (stated in this file's preamble). |
| 3 | Explicit note of any closed bead whose evidence looks weak or unverifiable, for downstream scrutiny | **This file, "Explicit weak / unverifiable-evidence section"** — 11 of 42 (5 weak-unverifiable + 6 partial) with a one-line why each, plus the 5 solid-with-caveat beads listed for completeness; ratings authoritative per irrevers-fdc1cd74, re-rated with per-bead confirmation by `docs/notes/evidence/closing-evidence-rerating.md` (child 4, irrevers-92165552), whose "Explicit weak / unverifiable-evidence section (current)" carries the same 5 + 6. |
| 4 | No edits to docs/plan/plan.md in this step — evidence gathering only | **Verified, and stated: `docs/plan/plan.md` was not edited by this certification step or by any bead in the irrevers-2c3b4637 chain.** Last commit touching the file is `d26a83a` (2026-09-11, the separate workflows-guard documentation bead irrevers-a39bdf35, outside this chain); the file is clean in the working tree at HEAD `3f0861f`. |

### Child-4 rating deltas, incorporated

Child 4 (irrevers-92165552, `docs/notes/evidence/closing-evidence-rerating.md`,
commit `c5dafab`, rated at HEAD `afaf476`) diffed its strong/partial/weak
re-rating of all 42 beads against this inventory as of `b15d587`:

- **Rating diff: empty — all 42 ratings confirm** (5 weak-unverifiable +
  6 partial + 31 strong, 5 with caveat), matching the baseline count for
  count. No rating in this inventory was moved, and the weak-evidence flags
  needed no refresh; both verification passes reported 0 unverifiable findings.
- **Five citation/timing corrections**, none of which moves a rating (child 4
  assessed each against the rating definitions). They are folded into the body
  rows above, replacing the superseded figures:
  1. **irrevers-e2bb8fbf** — `0fb164d` is an *earlier* related touch (an
     ancestor of `c7e9df5`), not a later one; `890429f` remains later.
  2. **irrevers-c87a3c50** — `src/trust_pointer.rs` first appears ~12.4 h
     after close, not ~9.5 h (direction unchanged; the longer gap makes the
     void-rehydration record more hollow).
  3. **irrevers-ff4f17da** — four conservative-trigger tests, not five:
     `:212` is the `release_is_fresh` helper. Re-verified directly at this
     HEAD: exactly four `#[test]` fns in src/rollback.rs.
  4. **irrevers-9007792b** — redaction-test anchor corrected `:1153` →
     **`:1158`** (`redacts_payloads_when_full_content_logging_is_disabled`).
     Third-order refinement: child 3's suggested `:1157` was itself off by
     one; `:1153` is blank at `e6771f0`, `c4385ac`, and HEAD. Re-verified at
     this HEAD. Downstream citations must use `:1158`.
  5. **irrevers-f61efd80** — `97b8eba` landed 36 min before close (not ~1 h);
     `48d5a60` landed 10 h 01 m after close (not ~9.6 h). The partial shape
     (close-time anchor covered only half the described scope; the rest
     postdates the close) is preserved exactly.
- **Clarification carried forward:** in the irrevers-49dbb095 caveat,
  `f0e0f9db` is a **bead ID**, not a SHA.

### Remaining gaps, for downstream scrutiny

No parent criterion is unsatisfied. These residuals stand, all already
recorded in the artifacts above; none is newly introduced by this chain:

1. **The 11 flagged closures remain unremediated** — 5 weak-unverifiable
   (c87a3c50 void rehydration; 340ae322, 93baa29a, b0a453b2, 29a9131c
   trust-the-notes-or-rerun) + 6 partial (eff8909f, f59f9313, 0f49129d,
   b4b37bf0, f61efd80, 6de781f4 — defective closure records, substance real).
2. **5 solid-with-caveat closures** carry scope/verification-shape caveats
   (84b36e47, 2cb3dbd2, 9eb4de16, ed77224f, 49dbb095).
3. **Product gap:** a pure redirect-channel flip is not flagged by the
   coverage-diff gate (irrevers-29a9131c residual, §C row).
4. **Prior-pass defects left in place by convention:** `final-inventory.md`'s
   stale icg-ci template line citations (130/149/156) and its non-summing
   count paragraph — recorded in this file's reconciliation §3, deliberately
   not edited in place (first-pass deliverable of record).

### Certification basis re-checked at this step (HEAD `3f0861f`)

- All 42 inventory IDs re-confirmed `closed` live via `bead list`.
- `e669d14` (this inventory's compile HEAD) and `b15d587` (child 4's diff
  baseline) both verified ancestors of HEAD — the chain's comparisons were
  made against states actually in this history.
- The two body corrections cheaply re-checkable from the repo
  (ff4f17da test count, denial_log.rs `:1158`) re-verified directly at this
  HEAD; the remaining three are adopted from child 4's recorded verification
  (child 2 `9fd67b6` / child 3 `8408665` — see
  `closing-evidence-rerating.md` Inputs for the per-pass HEADs).
- `docs/plan/plan.md` untouched by this step (see criterion 4 above).
