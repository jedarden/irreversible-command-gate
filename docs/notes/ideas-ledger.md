...
6. **Explicit fail-open/fail-closed decision for guard OOM-kill** (Lens 6, #42) — bead `icg-4bu`
   ...
   - **Complexity:** M
   - **First step:** Add a supervisor/watchdog that detects an OOM-killed check process and applies fail-open by default; wire it to `icg-2ck`'s poison-pill health signal so the default flips to fail-closed once reliability is validated.
